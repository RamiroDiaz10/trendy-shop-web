import { obtenerCarrito } from "../../services/cartService.js";

class NavBar extends HTMLElement {
  connectedCallback() {
    this.innerHTML = `
      <!-- Inicio navbar Daniel -->
      <nav class="navbar">
        <ul>
    ...
    window.addEventListener("carrito-actualizado", () => this.actualizarContador());
  }

  actualizarContador() {
    const carrito = obtenerCarrito();
    const totalItems = carrito.reduce((total, item) => total + item.cantidad, 0);
    this.querySelector("#carrito-contador").textContent = totalItems;
  }
}
